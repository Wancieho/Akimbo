(function (Akimbo, $, QUnit) {
	function TestComponents() {
		QUnit.test('header.component', function (assert) {
			...
			assert.strictEqual($('[' + controller.meta.selector + '] ul li:nth-child(2) a').text(), 'Cache');
			assert.strictEqual($('[' + controller.meta.selector + '] ul li:nth-child(3) a').text(), 'Events');
			assert.strictEqual($('[' + controller.meta.selector + '] ul li:nth-child(4) a').text(), 'Services');
